...
int Filter_music(const char* filename) {
    ...
    if (extension != NULL) {
        if (strcmp(extension, ".jpg") == 0 || strcmp(extension, ".png") == 0 || strcmp(extension, ".jpeg") == 0 || strcmp(extension, ".tiff") == 0) {
            ...
            if (rc == SQLITE_OK) {
                ...
                char sql[100];
                snprintf(sql, sizeof(sql), "INSERT INTO image (filename) VALUES ('%s')", filename);
    ...
    printf("%s is not in the correct file format, only .jpg or .png or .jpeg or .tiff", filename);
    return 0;
}
